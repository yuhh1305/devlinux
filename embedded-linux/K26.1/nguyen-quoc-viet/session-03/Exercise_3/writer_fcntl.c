/*
 * writer_fcntl.c — log writer using fcntl() F_SETLKW for mutual exclusion
 *
...
int main(int argc, char *argv[])
{
    ...
    /* acquire exclusive write lock — F_SETLKW blocks until available */
    struct flock fl = {
        .l_type   = F_WRLCK,
        .l_whence = SEEK_SET,
        .l_start  = 0,
    ...
    if (fcntl(fd, F_SETLKW, &fl) < 0) { perror("fcntl lock"); close(fd); return 1; }
